...
\item Sei
  ...
  \end{flalign*}
  \[
    \Mat\qty\big(\varphi_A, \mathcal{E}) = \begin{pmatrix}
      a & 0 & c & 0 \\
      0 & a & 0 & c \\
      b & 0 & d & 0 \\
      0 & b & 0 & d \\
...
\item Zeigen Sie: Für $A \in V$ gilt $f_{\varphi_A} = \qty\big(f_A)^2$.
  ...
  und
  \begin{flalign*}
    \det\qty\big(X \cdot I_4 - \Mat\qty\big(\varphi_A, \mathcal{E}))
    &= \qty\big(X - a_{11})\qty\big(X - a_{11})
      \qty\big(X - a_{22})\qty\big(X - a_{22}) + a_{12}^2 \cdot a_{21}^2\\
    &= \qty\Big(\qty\big(X - a_{11})\qty\big(X - a_{22}) + a_{12} \cdot a_{21})^2
  \end{flalign*}
