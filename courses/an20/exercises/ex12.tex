...
\item lokalle Extremstellen auf $\partial M$ ermitteln.
  ...
  \begin{flalign}
    \frac{\partial F(\ldots)}{\partial x} &= 2x (1 + \lambda) + y = 0 \label{eq:4} & \\
    \frac{\partial F(\ldots)}{\partial y} &= 2y (1 + \lambda) + x = 0 \label{eq:5} \\
    \frac{\partial F(\ldots)}{\partial \lambda} &= x^2 + y^2 - 1 = 0 \label{eq:6}
  \end{flalign}
  Angenommen es gilt $x = 0 \overset{\hyperref[eq:4]{(1)}}\Rightarrow y = 0
  \overset{\hyperref[eq:6]{(3)}}\Rightarrow -1 = 0$, Widerspruch.

  Angenommen es gilt $y = 0 \overset{\hyperref[eq:5]{(2)}}\Rightarrow x = 0
  \overset{\hyperref[eq:6]{(3)}}\Rightarrow -1 = 0$, Widerspruch.

  $\Rightarrow x \ne 0, y \ne 0$.
  \begin{flalign*}
    \hyperref[eq:4]{(1)} \cdot y &= \hyperref[eq:5]{(2)} \cdot x & \\
    2xy (1 + \lambda) + y^2 &= 0 = 2xy (1 + \lambda) + x^2 \\
    y^2 &= x^2 \iff \abs{y} = \abs{x} \iff y = \pm x \\
    \hyperref[eq:6]{(3)} &\Rightarrow h(x, \pm x) = 2x^2 - 1 = 0 \Rightarrow x_{1|2} = \pm \frac{1}{\sqrt{2}}
  \end{flalign*}
  \newpage
  Kritische Punkte:
  \begin{enumerate}[label={$\qty(x_{\arabic*}, y_{\arabic*}) = $}]
