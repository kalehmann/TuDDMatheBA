...
\item Geben Sie mit Hilfe der Exponentialdarstellung einer Zahl
  ...
  = \abs{z_1 z_2} e^{i\qty\big(\xi_1 + \xi_2)}$, dass heißt die Beträge werden
  multipliziert und $\xi_1, \xi_2$ addiert.
  ...
  \begin{tikzpicture}[scale=1.5]
    \begin{axis}[
      ...
      \draw[black!70!green] (0,0) -- (-0.4,0.4)
        node[below left = -3pt, black!70!green, pos=.5] {\tiny $\abs{z_2}$};
      \draw[black!70!green] (0.35,0) arc (0:135:0.35);

      \draw[black!50!red] (0,0) -- (-1.74,-0.57)
        node[below=4pt, black!50!red, pos=.5] {\small $\abs{z_1 + z_2}$};
  ...
  \begin{tikzpicture}[scale=1]
    \begin{axis}[
      ...
      \node[
        circle,
        fill,
        label=below right:{$\qty\big(\sqrt{3} + i)^{2022}$},
        inner sep=2pt
      ] at (-1,0) {};
...
\item Es seien $z, w \in \mathbb{C}$.
  ...
  \begin{flalign*}
    ...
    &= -i \sin\qty\big(-iz)\cos\qty\big(-iw) -
      i \sin\qty\big(-iw)\cos\qty\big(-iz) \\
    \overset{\text{Satz 13.22 (1)}}&=
      \sinh\qty\big(z)\cosh\qty\big(w) + \sinh\qty\big(w)\cosh\qty\big(z)
  \end{flalign*}
